...
const CLI_PATH = fileURLToPath(new URL("./pide.js", import.meta.url));
...
export const PIDE_MCP_TOOLS = [
  {
    name: "pide_context",
    description: "Return Pide ES providers, account configuration, comparison rules, and purchase boundaries. Call first.",
    inputSchema: objectSchema(), annotations: readOnly, command: () => ["context"],
  },
  {
    name: "pide_accounts_status",
  ...
  {
    name: "pide_provider_auth_login",
    description: "Open Glovo or Uber Eats on the official site in native Chrome. The user signs in there; no terminal, password, or cookie is requested in chat. Call pide_provider_auth_complete after the user says sign-in is finished.",
    inputSchema: objectSchema({
  ...
  {
    name: "pide_provider_auth_complete",
  ...
  {
    name: "pide_provider_auth_status",
  ...
  {
    name: "pide_search_begin",
    description: "Search all enabled Just Eat, Glovo, and Uber Eats accounts directly and rank matching offers. It never creates a basket.",
    inputSchema: objectSchema({
  ...
  },
  {
    ...
    inputSchema: objectSchema({
      searchId: string("Pide search ID."),
      provider: { type: "string", enum: ["glovo", "ubereats"] },
  ...
  {
    name: "pide_provider_error",
    description: "Mark one provider unavailable without blocking results from other enabled providers.",
    inputSchema: objectSchema({ searchId: string("Pide search ID."), provider: { type: "string", enum: ["justeat", "glovo", "ubereats"] }, message: string("Short error without personal data.") }, ["searchId", "provider", "message"]),
  ...
  },
  {
    ...
    inputSchema: objectSchema({ searchId: string("Pide search ID.") }, ["searchId"]), annotations: readOnly,
    command: (input) => ["search", "results", input.searchId, "--agent"],
  },
  {
    name: "pide_record_checkout_quote",
    ...
    inputSchema: objectSchema({
      searchId: string("Pide search ID."), offerId: string("Offer ID."),
  ...
  {
    name: "pide_open_basket",
    description: "Open the already-created Glovo or Uber Eats basket in official native-browser checkout for manual review. Never presses the final purchase button.",
    inputSchema: objectSchema({ searchId: string("Pide search ID."), offerId: string("Offer ID.") }, ["searchId", "offerId"]), annotations: remoteWrite,
    command: (input) => ["basket", "open", input.searchId, input.offerId, "--agent"],
  },
  {
    name: "pide_place_order",
    ...
    inputSchema: objectSchema({ searchId: string("Pide search ID."), offerId: string("Offer ID."), confirm: string("Exact fingerprint returned by the immediately preceding dry run, only after explicit approval.") }, ["searchId", "offerId"]), annotations: purchaseWrite,
    command: (input) => ["order", "place", input.searchId, input.offerId, ...(input.confirm ? ["--confirm", input.confirm] : []), "--agent"],
  },
];

function toolByName(name) { return PIDE_MCP_TOOLS.find((tool) => tool.name === name); }

export async function executePideMcpTool(name, input = {}) {
  const tool = toolByName(name);
  if (!tool) throw new Error(`Unknown Pide tool: ${name}`);
...
export async function handlePideMcpMessage(message) {
  const id = message.id ?? null;
  if (message.method === "initialize") return { jsonrpc: "2.0", id, result: { protocolVersion: "2025-03-26", capabilities: { tools: {} }, serverInfo: { name: "pide-es", version: "0.1.0" } } };
  if (message.method === "notifications/initialized") return null;
  if (message.method === "tools/list") return { jsonrpc: "2.0", id, result: { tools: PIDE_MCP_TOOLS.map(({ command, ...tool }) => tool) } };
  if (message.method === "tools/call") {
    try { return { jsonrpc: "2.0", id, result: await executePideMcpTool(message.params?.name, message.params?.arguments ?? {}) }; }
...
export async function runPideMcpServer({ input = process.stdin, output = process.stdout } = {}) {
  const lines = createInterface({ input, crlfDelay: Infinity });
  for await (const line of lines) {
    if (!line.trim()) continue;
    let response;
    try { response = await handlePideMcpMessage(JSON.parse(line)); }
