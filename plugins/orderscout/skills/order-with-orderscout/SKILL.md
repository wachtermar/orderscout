...
1. Read the complete request and build two small query lists before calling `orderscout_search_begin`:
   - `discoveryQueries`: broad merchant, cuisine, and store-category terms, including useful Spanish/local synonyms. Examples: `vape`, `vaper`, `estanco`; `healthy`, `poke`, `mediterranean`; `farmacia`, `parafarmacia`; `supermercado`.
   - `catalogQueries`: the required item or dish form plus preference terms, again with useful local synonyms. Examples: `liquid`, `líquido`, `recarga`, `ice`, `hielo`, `mentol`; `pollo a la parrilla`, `ensalada`, `bowl`.
   Keep required form separate from optional preference. In “vape liquid, preferably ice,” liquid is required while ice is a ranking preference. In “healthy but tasty dinner,” both are quality preferences over actual complete meals.
2. Call `orderscout_search_begin` once with the user's complete wording unchanged, the objective, both query lists, and a location only when the adapters cannot use a saved address. Request enough normalized candidates for meaningful reasoning—usually 30–60 for a broad request. Do not select providers in the search call; the CLI fans out to every enabled account concurrently.
3. The CLI performs provider-native retrieval. Glovo first finds relevant merchant cards and then queries each relevant shop's full catalog search index; Just Eat and Uber Eats expand merchant results through their menu/catalog adapters. It merges the LLM plan with deterministic fallback terms, normalizes untrusted product data, and enforces strict relevance. The LLM then judges the resulting products, combinations, trade-offs, and request-specific quality. Do not ask the model to ingest raw provider payloads or thousands of untrusted catalog entries.
4. Dates such as “tomorrow at 10am” must appear as `search.fulfilment.requestedAt`; occasions such as breakfast must appear as `parsedIntent.occasion`. If either is missing or wrong, stop and report a parsing defect—do not continue with current-time or generic-meal results.
5. Inspect `coverage` and every provider status. Do not present a cross-provider result until `allConfiguredAttempted` is true. State any provider in `failedProviders` or `partialProviders`; never silently omit it or replace it with browser search. A partial or failed catalog scan is not proof that the provider has no match. Retry a transient partial search once with a narrower, high-signal plan when useful, otherwise report the exact coverage limitation.
6. Call `orderscout_results`, increasing `limit` when the first shortlist is too narrow for the user's qualitative request. Always show Just Eat, Glovo, and Uber Eats status explicitly: matched, no suitable match, partial, failed, disabled, or not authenticated. Never describe “all apps” while omitting a configured provider.

The CLI's deterministic layer is a safety and recall guard, not the final decision maker. Use the LLM to choose search vocabulary, recognize equivalent products and dishes, compose sensible multi-item meals, interpret deals and ratings, and explain why one option fits the user better. Never invent menu facts or relax a required product form merely to return a result.

For arbitrary products, trust the normalized whole-concept filtering and never supplement it with browser search. Treat `coverage.availableProviders` as currently orderable matches and `coverage.unavailableOnlyProviders` as relevant catalog matches that cannot be ordered now. A preference match never proves product relevance. State the actual product form shown by the offer—for example, do not call a disposable vape “e-liquid.” A closed but schedulable store is an unavailable-now match, not “no match.”
