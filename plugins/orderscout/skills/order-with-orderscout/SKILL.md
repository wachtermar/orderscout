...
The Browser is limited to two handoffs: the user completing login on an official provider page opened by an OrderScout auth tool, and optional visual review or manual editing of an already-prepared official checkout. Never manually inspect or expose browser cookies, local storage, passwords, tokens, profiles, or session stores; the CLI's owner-only native session importer handles the narrow provider data it needs.
