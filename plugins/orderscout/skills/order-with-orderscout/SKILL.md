...
7. For every provider with a suitable option, call `orderscout_select_candidates` with the best same-merchant bundle for that provider. Map each line with `forItem`, quantity, and a grounded `reason`. For multi-person meals, select distinct suitable dishes unless one item explicitly serves the party. The CLI validates IDs, quantities, and basket compatibility only; selection is local and does not alter a provider cart.
8. Call `orderscout_results` after selections. It ranks only model-selected bundles. If `candidatePool.selectionRequired` is true, selection is incomplete: do not report “no match,” a winner, or a recommendation. Always state Just Eat, Glovo, and Uber Eats status explicitly as selected match, inspected no suitable candidate, partial, failed, disabled, or not authenticated.
