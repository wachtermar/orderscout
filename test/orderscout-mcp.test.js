...
test("OrderScout MCP exposes direct login, basket, checkout, and guarded order tools", () => {
  ...
  assert.match(ORDERSCOUT_MCP_TOOLS.find((tool) => tool.name === "orderscout_provider_browser_session").description, /does not authenticate the OrderScout CLI/);
  assert.match(ORDERSCOUT_MCP_TOOLS.find((tool) => tool.name === "orderscout_accounts_status").description, /cached state, not proof/);
