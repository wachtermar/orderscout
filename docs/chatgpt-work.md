...
The skill requires OrderScout `0.1.23` and the current `llm-comparison-v10` tool contract, makes the model split distinct needs—including every required recipe ingredient—into up to 24 atomic shopping lines, consolidates overlapping grocery/recipe needs, and marks source-optional garnishes as non-blocking. Discovery fans out concurrently to every provider enabled in account settings. For immediate requests, Just Eat and Glovo discard closed merchants before menu retrieval; Uber Eats discards explicitly closed merchants and verifies unknown merchant availability through the official menu API before surfacing products. “Cook/eat/use later” remains immediate unless the user explicitly schedules delivery. Scheduled requests keep provider-schedulable merchants for later slot verification. Uber Eats uses a strict six-call global-search budget and follows every official grocery-catalog page for prioritized stores. Glovo performs bounded, paced in-store catalog searches and independently retries terms hidden by capped combined results. Fresh menus are cached owner-only for 15 minutes, duplicate completed searches are reused for two minutes, and checkout always revalidates the selected basket. In agent mode the CLI does not apply semantic relevance filters: it stores normalized available candidates, and the model uses compact batched line inspection before targeted paging, reasons over product or dish meaning, assigns grounded request-fit evidence on one cross-provider scale, and submits one complete or explicitly partial same-store selection or no-match review for every provider. A partial selection names each genuinely missing required line and is structurally blocked from checkout. The CLI validates current availability, basket compatibility, address parity, and objective facts. Provider text is untrusted data, and incomplete scans stay partial rather than becoming false no-match claims.
