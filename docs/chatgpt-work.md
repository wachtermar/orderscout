...
The skill makes the model create a bounded merchant-discovery plan and a separate in-catalog product plan from the user's natural-language request. Discovery then fans out concurrently to every provider enabled in account settings. Glovo expands matching merchant cards through the relevant shops' catalog indexes instead of assuming the global search contains products; the other adapters expand direct menus. Only normalized product records reach model reasoning, and provider text is untrusted data. Coverage records incomplete catalog scans as partial rather than turning them into false no-match claims.
