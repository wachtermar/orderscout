...
The skill requires OrderScout `0.1.17` and the current `llm-comparison-v7` tool contract, makes the model split distinct needs into shopping lines, and creates fair bounded merchant-discovery and catalog plans. Discovery fans out concurrently to every provider enabled in account settings. Just Eat scans every currently eligible area menu, Glovo loads complete menus for discovered merchants, and Uber Eats uses a rate-aware representative query plan that preserves at least one focused query per shopping line before staggered prioritized full-menu expansion. In agent mode the CLI does not apply semantic relevance filters: it stores normalized candidates, and the model pages through relevant subsets, reasons over product or dish meaning, assigns grounded request-fit evidence on one cross-provider scale, and submits one same-store selection or explicit no-match review for every provider. The CLI validates current availability, basket compatibility, address parity, and objective facts. Provider text is untrusted data, and incomplete scans stay partial rather than becoming false no-match claims.
