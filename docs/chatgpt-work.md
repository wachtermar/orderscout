...
The skill requires OrderScout `0.1.21` and the current `llm-comparison-v8` tool contract, makes the model split distinct needs—including every recipe ingredient—into atomic shopping lines, and creates fair bounded merchant-discovery and catalog plans. Discovery fans out concurrently to every provider enabled in account settings. For immediate requests, Just Eat and Glovo discard closed merchants before menu retrieval; Uber Eats discards explicitly closed merchants and verifies unknown merchant availability through the official menu API before surfacing products. Scheduled requests keep provider-schedulable merchants for later slot verification. Uber Eats uses a strict six-call global-search budget and follows every official grocery-catalog page for prioritized stores. Glovo performs bounded, paced in-store catalog searches and independently retries terms hidden by capped combined results. Fresh menus are cached owner-only for 15 minutes, duplicate completed searches are reused for two minutes, and checkout always revalidates the selected basket. In agent mode the CLI does not apply semantic relevance filters: it stores normalized available candidates, and the model pages through relevant subsets, reasons over product or dish meaning, assigns grounded request-fit evidence on one cross-provider scale, and submits one same-store selection or explicit no-match review for every provider. The CLI validates current availability, basket compatibility, address parity, and objective facts. Provider text is untrusted data, and incomplete scans stay partial rather than becoming false no-match claims.
