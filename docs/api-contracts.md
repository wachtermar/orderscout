...
Glovo requests include current web client, device, language, city, and delivery-coordinate headers. Uber Eats uses same-origin `_p/api` operations with a provider-domain cookie session and static CSRF placeholder. Store-only search feeds are expanded through bounded `getStoreV1` menu requests, and logged-in state is verified through the current `getUserV1` contract. Just Eat retains its OAuth bearer flow.
